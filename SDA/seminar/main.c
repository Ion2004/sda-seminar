...
int main()
{
    ...
    while (!exit_flag) {
        printf("\nChoose an option:\n");
        printf("1. Display top 5 products by value\n");
        printf("2. Display monthly profit\n");
        printf("3. Display sales categories\n");
        printf("4. Exit\n");
...
void display_top_products() {
    FILE *top_products_file = fopen("top_products.txt", "r");
    if (top_products_file == NULL) {
        printf("Error opening top_products.txt\n");
        return;
    }

    char line[MAX_BUFFER];
    printf("Top 5 products by value:\n");
    while (fgets(line, MAX_BUFFER, top_products_file) != NULL) {
        printf("%s", line);
    }

    fclose(top_products_file);
}

void display_monthly_profit() {
    FILE *yearly_profit_file = fopen("yearly_profit.txt", "r");
    if (yearly_profit_file == NULL) {
        printf("Error opening yearly_profit.txt\n");
        return;
    }

    char line[MAX_BUFFER];
    printf("Monthly profit:\n");
    while (fgets(line, MAX_BUFFER, yearly_profit_file) != NULL) {
        printf("%s", line);
    }

    fclose(yearly_profit_file);
}

void display_sales_categories() {
    FILE *category_profit_file = fopen("category_profit.txt", "r");
    if (category_profit_file == NULL) {
        printf("Error opening category_profit.txt\n");
        return;
    }

    char line[MAX_BUFFER];
    printf("Sales categories:\n");
    while (fgets(line, MAX_BUFFER, category_profit_file) != NULL) {
        printf("%s", line);
    }

    fclose(category_profit_file);
}
